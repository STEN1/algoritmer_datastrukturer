...
QuadNode::QuadNode(const Rect& rect, QuadNode* parent)
	:rect_(rect), sv_(nullptr), so_(nullptr), no_(nullptr), nv_(nullptr), parent_(parent), max_gameobjects(8)
{
}
...
std::pair<QuadNode*, bool> QuadNode::insert_gameobject2(GameObject* gameobject)
{
	...
	for (auto node : childnodes_)
	{
		if (node->rect_.intersect(gameobject->pos))
		{
			return node->insert_gameobject2(gameobject);
		}
	}
	...
	if (gameobjects_.size() == max_gameobjects)
	{
		...
		gameobjects_.clear();
		return insert_gameobject2(gameobject);
	}
